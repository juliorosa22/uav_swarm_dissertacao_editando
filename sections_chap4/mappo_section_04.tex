...
Esta seção descreve a implementação prática do algoritmo \textit{Multi-Agent Proximal Policy Optimization} (MAPPO). Em ambos os casos, a formulação do algoritmo segue como base o trabalho de referência apresentado em \citeonline{yu2021mappo}. No entanto, diferenças de implementação são observadas em função das bibliotecas de aprendizado por reforço adotadas em cada simulador, bem como das particularidades da integração entre o algoritmo e o ambiente de simulação.
...
\subsection{Implementação no AirSim}
\label{subsec:mappo_airsim_torchrl}

% \subsubsection*{\textbf{Desafios na Integração entre Simulador e Algoritmo}}
...
\subsubsection*{\textbf{Arquitetura das Redes Neurais}}
...
% Nodes
\node[box] (obs) {Observação do Agente\\
\scriptsize (estado cinemático,\\ obstáculos, vizinhos, RM) \\ (23x1)};
\node[box, below=of obs] (fc1) {MLP\\
128 neurônios + ReLU};
\node[box, below=of fc1] (fc2) {MLP\\
128 neurônios + ReLU};
\node[box, below=of fc2] (out) {Ações Contínuas\\
\scriptsize (4x1: empuxo e momentos)};
...
\end{tikzpicture}
\caption{Arquitetura da rede do ator utilizada no ambiente IsaacLab. A política é descentralizada e compartilhada entre os agentes, operando apenas sobre observações vetoriais individuais.}
\label{fig:mappo_actor_isaaclab}
\end{figure}



O crítico adota uma arquitetura mais profunda, composta por três camadas ocultas de dimensões
256, 256 e 128 neurônios. Sob o paradigma de treinamento centralizado, sua entrada é formada pela
...
% Nodes
\node[box] (state) {Estado Global\\
\scriptsize Concatenação das\\ observações dos agentes\\ (N x 23)};
\node[box, below=of state] (fc1) {MLP\\
256 neurônios + ReLU};
\node[box, below=of fc1] (fc2) {MLP\\
256 neurônios + ReLU};
\node[box, below=of fc2] (fc3) {MLP\\
128 neurônios + ReLU};
\node[box, below=of fc3] (value) {Valor do Estado\\
$V(s)$};
...
\draw[arrow] (fc1) -- (fc2);
\draw[arrow] (fc2) -- (fc3);
\draw[arrow] (fc3) -- (value);

\end{tikzpicture}
\caption{Arquitetura da rede do crítico no ambiente IsaacLab. O crítico opera sob o paradigma de treinamento centralizado, estimando o valor do estado global a partir da concatenação direta das observações individuais dos agentes.}
\label{fig:mappo_critic_isaaclab}
\end{figure}
