...
\delta\big(u_i(t), L(\mathbf{o}_i(t))\big).
\]
...
Para facilitar a interpretação dessa dinâmica, a Figura~\ref{fig:rm_machine} apresenta
o diagrama de estados correspondente à Máquina de Recompensa adotada neste trabalho.
No diagrama, cada nó representa um estado lógico da RM e cada transição é rotulada por
uma tupla composta pela proposição lógica avaliada no ambiente e pela configuração de
pesos da função de recompensa ativada naquele estado. Essa representação gráfica
evidencia de forma compacta a relação entre condições ambientais, estados da RM e
a modulação dinâmica da função de recompensa.

%%Inserir um texto explicativo sobre a figura da máquina de recompensa
\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{fig/rm_isaacsim.pdf}
    \caption{Máquina de Recompensa (RM) utilizada no ambiente IsaacSim. 
    Cada transição é rotulada por uma tupla composta pela proposição lógica
    avaliada no ambiente e pela configuração de pesos ativada na função de recompensa.}
    \label{fig:rm_machine}
\end{figure}
